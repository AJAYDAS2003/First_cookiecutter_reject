...
def main():
    try:
        ...
        logger.info(f"Max features for TfidfVectorizer: {max_features}")
        ...
        train_df.to_csv(os.path.join(data_path, 'train_tfidf.csv'), index=False, encoding='utf-8')
        test_df.to_csv(os.path.join(data_path, 'test_tfidf.csv'), index=False, encoding='utf-8')

        logger.info("TF-IDF feature engineering completed and CSV files saved.")
        ...
        logger.exception(f"Unexpected error in TF-IDF feature engineering pipeline: {e}")
